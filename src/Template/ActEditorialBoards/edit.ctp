...
?>
<div class="container">
    <?= $this->Form->create($actEditorialBoard, ['templates' => ['inputContainer' => '<div class="col-md-6"><div class="form-group" >{{content}}</div></div>'], 'name' => 'edit']) ?>
        <div class="row">
            <legend><?= __('Edit Editorial Board') ?></legend>
            <?php
                echo $this->Form->control('journal_name', ['label'=> __('Journal name'), 'class' => 'form-control', 'pattern' => '[A-Za-záéíóúüñ ]{3,100}']);
