...
?>
<div class="contanier">
    <?= $this->Form->create($pubBook, ['templates' => ['inputContainer' => '<div class="col-md-6"><div class="form-group" >{{content}}</div></div>'], 'name' => 'edit']) ?>
        <div class="row">
            ...
            <?php
                ...
                echo $this->Form->control('editorial', ['class' => 'form-control', 'pattern' => '[A-Za-záéíóúüñ ]{3,100}']);
                echo $this->Form->control('year', ['class' => 'form-control', 'pattern' => '[0-9]{4}', 'label' => __('Year').' ('.__('yyyy').')']);
                echo $this->Form->control('country', ['label'=> __('Country'), 'class' => 'form-control', 'pattern' => '[A-Za-záéíóúüñ ]{3,45}']);
